...
import { ECONOMIC_CATEGORIES, IN_SCOPE_ACCOUNTS, REFUND_KINDS, labelFor, costRelevance, type EconomicCategory } from '@/apps/auto-sales/types'
...
export default async function VehicleFolderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Display split (facts already in the ledger): gross added costs vs returns/credits.
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-gray-200 max-w-xl mx-auto px-4 py-6">
      <Link href="/admin/auto-sales" className="text-gray-500 text-sm">← Inventory</Link>
          ...
          {result.unresolved.length > 0 && (
            <div className="mt-3 rounded-xl border border-amber-900/50 bg-amber-950/15 p-3">
              ...
              <details className="mt-2"><summary className="text-gray-400 text-xs cursor-pointer">Update ▾</summary>
                <form action={closeout} className="grid grid-cols-2 gap-2 mt-2">
      ...
      <div className="mt-5 space-y-3">
        ...
        <details className="rounded-2xl bg-gray-900 border border-gray-800 overflow-hidden">
          ...
          <form action={addExpense} className="px-4 pb-4 space-y-3">
            <input type="hidden" name="inventoryVehicleId" value={inv.id} />
        ...
        {!sold && (
          <details className="rounded-2xl bg-gray-900 border border-gray-800 overflow-hidden">
            ...
            <form action={sell} className="px-4 pb-4 space-y-3">
              <input type="hidden" name="inventoryVehicleId" value={inv.id} />
              <div className="grid grid-cols-2 gap-3">
        ...
        {returnable.some((r) => r.remainingCents > 0) && (
          <details className="rounded-2xl bg-gray-900 border border-gray-800 overflow-hidden">
            ...
            <form action={returnRefund} className="px-4 pb-4 space-y-3">
              <input type="hidden" name="inventoryVehicleId" value={inv.id} />
              ...
              <div key={e.id} className="flex items-center justify-between text-sm">
                ...
                <div className="text-right">
                  ...
                  {e.refundStatus !== 'settled' && (e.refundMethod === 'cash' || e.refundMethod === 'card') && (
                    <form action={settle} className="mt-1 flex items-center gap-1 justify-end"><input type="hidden" name="inventoryVehicleId" value={inv.id} /><input type="hidden" name="eventId" value={e.id} /><input name="date" type="date" defaultValue={today} className="bg-gray-800 border border-gray-700 rounded px-2 py-1 text-xs text-white" required /><button className="text-emerald-400 text-xs underline">received</button></form>
                  )}
                </div>
              </div>
            ))}
      ...
      {/* History (ledger) — hidden by default */}
            ...
            return (
              <div key={e.id} className={`flex items-center justify-between text-sm border-b border-gray-800/60 pb-1.5 ${voided ? 'opacity-45 line-through' : ''}`}>
                ...
                <div className="text-right shrink-0">
                  ...
                  {!voided && !['acquisition', 'sale', 'return', 'refund', 'vendor_credit'].includes(e.economicCategory) && <form action={reverse} className="inline"><input type="hidden" name="inventoryVehicleId" value={inv.id} /><input type="hidden" name="eventId" value={e.id} /><button className="text-gray-600 text-[11px] underline ml-2">undo</button></form>}
                </div>
              </div>
            )
          })}
          <p className="text-gray-600 text-[11px] pt-1">Nothing here is ever deleted. “Undo” adds a correcting entry; returns stay linked to the original purchase.</p>
        </div>
      </details>

      {/* Details / advanced — admin/accounting facts, hidden */}
