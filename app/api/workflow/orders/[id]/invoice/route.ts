...
import { getBusinessConfig, retailQbEnabled } from '@/apps/settings/db'
...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [full, cfg, qbEnabled] = await Promise.all([getFullEstimate(id), getBusinessConfig(), retailQbEnabled()])
  return NextResponse.json({ ok: true, qbEnabled, draft: buildInvoiceDraft({ order, full, paymentLabel: cfg.paymentLabel, role: actor.role }) })
}
